...
const BusinessOrders = () => {
  ...
  const getSessionData = async () => {
    try {
      ...
      if (sessionString) {
        ...
        if (sessionData.shop?.name) {
          setShopName(sessionData.shop.name);
        }
      }
    } catch (error) {
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      const response = await axios.get('https://leafeye.eu-1.sharedwithexpose.com/api/business/orders');
  ...
  const handleStatusChange = async (orderId: number, newStatus: string) => {
    try {
      ...
      const response = await axios.patch(`https://leafeye.eu-1.sharedwithexpose.com/api/business/orders/${orderId}/status`, {
        status: newStatus
      });
              ...
              <View key={index} style={styles.orderCard}>
                <View style={styles.orderHeader}>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => handleStatusChange(order.id, order.status === 'completed' ? 'pending' : 'completed')}
                ...
                <View style={styles.customerInfo}>
                  ...
                  <Text style={styles.customerDetail}>{order.user.address}</Text>
                </View>
                ...
                <View style={styles.orderFooter}>
                  <Text style={styles.orderDate}>Ordered: {new Date(order.date_ordered).toLocaleDateString()}</Text>
                  <Text style={styles.totalAmount}>
                    Total: EGP {order.products.reduce((sum, product) => sum + (product.price * product.quantity), 0).toFixed(2)}
                  </Text>
                </View>
              </View>
            ))}
